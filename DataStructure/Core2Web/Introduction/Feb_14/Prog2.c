...
// Since passing whole structure to a function is a bit heavy operation as copy is performed so we can take pointer to structure of cric and fb object and instead of passing whole structure we can pass these pointers
...
void main() {
	...
	struct Player* cricptr = &cric;
	struct Player* fbptr = &fb;
